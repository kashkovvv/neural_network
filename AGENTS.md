...
Текущий арифметический контракт использует только `std::floating_point<T>`, exact shape для tensor–tensor операций и обычное native floating-point поведение. Broadcasting ещё не реализован. Обычное число поддерживается отдельными scalar-перегрузками; rank-zero `Tensor` с тензором другой формы будет обрабатываться позднее общим broadcasting-механизмом, а не отдельными специальными перегрузками.

Следующий этап — `reshape`. Работа над его контрактом ещё не начиналась. Не считай прежние идеи о mutating или value-returning API согласованными: сначала восстанови текущие инварианты и отдельно предложи контракт.
...
После согласования контракта `reshape` сам напиши тесты, покрывающие как минимум:

- сохранение `numel()` и линейного порядка элементов;
- корректные новые shape и row-major strides;
- изменение rank при одинаковом количестве элементов;
- scalar, singleton и zero-extent формы;
- переходы между scalar и одноэлементными формами;
- несколько разных zero-extent форм с `numel() == 0`;
- отклонение формы с другим количеством элементов;
- переполнение произведения extents и превышение допустимого размера storage;
- поведение moved-from sentinel;
- mutable/const и lvalue/rvalue категории согласно принятому контракту;
- заявленную exception safety и отсутствие изменений после ошибки;
- точный возвращаемый тип и `noexcept`-свойства на этапе компиляции.
...
3. Кратко зафиксируй инварианты shape/strides/storage, scalar, zero-extent и moved-from sentinel, которые должен сохранять `reshape`.
4. Объясни назначение `reshape` и предложи лучший контракт для этого проекта: mutating или value-returning API, mutable/const и lvalue/rvalue категории, поведение scalar и zero-extent, проверку равенства `numel`, порядок элементов, типы ошибок, exception safety и `noexcept`.
5. Сравни реальные альтернативы, выбери одну и остановись до моего подтверждения. Пока не пиши production-реализацию и не создавай тесты.

Дальше продолжай строго по описанному процессу: после согласования тесты пишешь ты, production-код пишу я, а затем ты проводишь максимально дотошное ревью.
