...
обычное native floating-point поведение. Broadcasting реализуется поэтапно:
right-aligned broadcasting правого tensor-операнда в неизменную форму левого
завершён для `operator+=` и `operator-=`. Обычное число поддерживается отдельными
scalar-перегрузками; rank-zero `Tensor` обрабатывается общим
broadcasting-механизмом, а не специальной арифметической перегрузкой.

Не считай broadcasting завершённым после одного compound-оператора. До отметки
этапа необходимо согласованно реализовать остальные tensor–tensor compound и
симметричные бинарные операции, покрыть их тестами и провести полное ревью.
...
broadcasting для `operator+=` и `operator-=` реализован через общий private
kernel `apply_elementwise_inplace`; его актуальное состояние сначала восстанови
по Git diff и тестам.

Следующими последовательно подключай к тому же kernel `operator*=` и
`operator/=`, каждый раз сначала добавляя тесты и не дублируя offset mapping.
Затем отдельно спроектируй симметричный binary kernel: текущая модель бинарного
оператора через копию левого operand не умеет создавать результат, форма
которого шире формы слева. До завершения симметричных `+`, `-`, `*`, `/`
broadcasting нельзя считать законченным и нельзя отмечать общий пункт README.
