...
текущий подэтап — right-aligned broadcasting правого tensor-операнда в
неизменную форму левого для `operator+=`. Обычное число поддерживается отдельными
...
broadcasting для `operator+=` реализуется через общий private kernel
`apply_elementwise_inplace`; его актуальное состояние сначала восстанови по Git
diff и тестам.

После принятия этого подэтапа последовательно подключай к тому же kernel
`operator-=`, `operator*=` и `operator/=`, каждый раз сначала добавляя тесты и не
дублируя offset mapping. Затем отдельно спроектируй симметричный binary kernel:
текущая модель бинарного оператора через копию левого operand не умеет создавать
результат, форма которого шире формы слева. До завершения симметричных `+`, `-`,
`*`, `/` broadcasting нельзя считать законченным и нельзя отмечать общий пункт
README.
