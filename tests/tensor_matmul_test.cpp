...
void test_matmul_rejects_non_matrix_operands() {
  ...
  const Tensor rank_three = Tensor({1, 2, 2});
  ...
  expect_throws<std::invalid_argument>(
      [&rank_three, &valid_matrix] {
        static_cast<void>(rank_three.matmul(valid_matrix));
      },
      "matmul accepted a rank-three left operand",
      "matmul with a rank-three left operand produced the wrong exception "
      "type");
  expect_throws<std::invalid_argument>(
      [&valid_matrix, &scalar] {
  ...
  expect_throws<std::invalid_argument>(
      [&valid_matrix, &rank_three] {
        static_cast<void>(valid_matrix.matmul(rank_three));
      },
      "matmul accepted a rank-three right operand",
      "matmul with a rank-three right operand produced the wrong exception "
      "type");
}
...
int main() {
  try {
    ...
    test_matmul_rejects_non_matrix_operands();
