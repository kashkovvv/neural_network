...
static_assert(std::same_as<decltype(std::declval<const Tensor&>() *
                           ...
                           Tensor>);
static_assert(CanCallFreeMultiply<const Tensor&, const Tensor&>);
static_assert(CanMultiply<Tensor&, const Tensor&>);
static_assert(CanMultiply<const Tensor&, const Tensor&>);
static_assert(CanMultiply<Tensor&&, const Tensor&>);
static_assert(CanMultiply<const Tensor&, Tensor&&>);
static_assert(CanMultiply<DoubleTensor&, const DoubleTensor&>);
static_assert(!CanMultiply<IntegerTensor&, const IntegerTensor&>);
static_assert(!CanMultiply<Tensor&, const DoubleTensor&>);
static_assert(
...
void test_multiply_computes_elementwise_without_changing_lvalues() {
  const Tensor left =
  ...
  const Tensor::storage_type expected_result{6.0F, -4.0F, -12.0F,
                                              2.0F, 4.0F, 3.0F};
  const Tensor::storage_type expected_left{1.0F, 2.0F, -3.0F,
                                            4.0F, 0.5F, -1.0F};
  const Tensor::storage_type expected_right{6.0F, -2.0F, 4.0F,
                                             0.5F, 8.0F, -3.0F};

  const Tensor result = left * right;

  expect_state(result, expected_shape, expected_strides, expected_result,
               "multiplication produced an incorrect shape",
               "multiplication produced incorrect strides",
               "multiplication produced incorrect elements");
  expect(std::ranges::equal(left.elements(), expected_left),
         "multiplication changed the left lvalue elements");
...
void test_multiply_supports_scalars_and_zero_extent_tensors() {
  ...
  expect(scalar_result.rank() == 0, "scalar multiplication changed rank");
  expect(scalar_result.numel() == 1, "scalar multiplication changed numel");
  expect(scalar_result.at() == -5.0F,
         "scalar multiplication produced an incorrect value");
...
void test_multiply_supports_aliased_lvalues_without_changing_the_source() {
  ...
  const Tensor::storage_type expected_result{1.0F, 4.0F, 12.25F, 0.0F};

  const Tensor result = tensor * tensor;

  expect(std::ranges::equal(result.elements(), expected_result),
         "multiplication of aliased lvalues computed incorrect elements");
  expect(std::ranges::equal(tensor.elements(), expected_source),
         "multiplication of aliased lvalues changed the source");
}

void test_multiply_reuses_rvalue_left_storage_and_consumes_it() {
  Tensor left = Tensor::from_data({3}, {1.0F, -2.0F, 3.5F});
  const Tensor right = Tensor::from_data({3}, {4.0F, -3.0F, 2.0F});
  const Tensor::value_type* const original_storage = left.elements().data();
  const Tensor::storage_type expected_elements{4.0F, 6.0F, 7.0F};

  const Tensor result = std::move(left) * right;

  expect(result.elements().data() == original_storage,
         "multiplication did not reuse the rvalue left storage");
  expect(std::ranges::equal(result.elements(), expected_elements),
         "multiplication with an rvalue left computed incorrect elements");
...
void test_multiply_does_not_consume_rvalue_right() {
  ...
  expect(std::ranges::equal(result.elements(), expected_result),
         "multiplication with an rvalue right computed incorrect elements");
  expect(std::ranges::equal(right.elements(), expected_right),
...
void test_multiply_rejects_incompatible_shapes_without_changing_lvalues() {
  ...
  expect_throws<std::invalid_argument>(
      ...
      "multiplication with different shapes did not throw "
      "std::invalid_argument",
      "multiplication with different shapes produced the wrong exception "
      "type");
...
void test_failed_multiply_consumes_rvalue_left() {
  ...
  expect_throws<std::invalid_argument>(
      ...
      "failed multiplication with an rvalue left did not throw",
      "failed multiplication with an rvalue left produced the wrong exception "
      "type");

  expect_empty_sentinel(left);
}

void test_multiply_rejects_moved_from_sentinels() {
  ...
  expect_throws<std::invalid_argument>(
      ...
      "multiplication accepted a sentinel left operand",
      "sentinel left multiplication produced the wrong exception type");
  ...
  expect_throws<std::invalid_argument>(
      ...
      "multiplication accepted a sentinel right operand",
      "sentinel right multiplication produced the wrong exception type");
...
int main() {
  ...
  std::cout << "All tensor multiplication tests passed\n";
  return EXIT_SUCCESS;
}
