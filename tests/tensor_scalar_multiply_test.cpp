...
concept CanMultiply = requires {
  std::declval<Left>() * std::declval<Right>();
};

template <typename Left, typename Right>
concept CanCallFreeMultiply = requires {
...
static_assert(std::same_as<decltype(std::declval<float>() *
                           ...
                           Tensor>);
static_assert(CanCallFreeMultiply<const Tensor&, float>);
static_assert(CanCallFreeMultiply<float, const Tensor&>);
static_assert(CanMultiply<DoubleTensor&, double>);
static_assert(CanMultiply<double, DoubleTensor&>);
static_assert(CanMultiply<Tensor&, int>);
static_assert(CanMultiply<int, Tensor&>);
static_assert(CanMultiply<Tensor&&, float>);
static_assert(CanMultiply<float, Tensor&&>);
static_assert(!CanMultiply<IntegerTensor&, int>);
static_assert(!CanMultiply<int, IntegerTensor&>);
...
void test_scalar_multiply_supports_both_orders_without_changing_lvalue() {
  const Tensor tensor =
  ...
  expect_state(tensor_first, expected_shape, expected_strides,
               expected_elements, "tensor-first scalar multiplication changed shape",
               "tensor-first scalar multiplication changed strides",
               "tensor-first scalar multiplication computed incorrect elements");
  expect_state(value_first, expected_shape, expected_strides,
               expected_elements, "value-first scalar multiplication changed shape",
               "value-first scalar multiplication changed strides",
               "value-first scalar multiplication computed incorrect elements");
  expect(std::ranges::equal(tensor.elements(), expected_source),
         "scalar multiplication changed the tensor lvalue");
}

void test_scalar_multiply_accepts_convertible_numbers_in_both_orders() {
  ...
  expect(std::ranges::equal(tensor_first.elements(), expected_elements),
         "tensor-first scalar multiplication converted a number incorrectly");
  expect(std::ranges::equal(value_first.elements(), expected_elements),
         "value-first scalar multiplication converted a number incorrectly");
}

void test_scalar_multiply_supports_rank_zero_and_zero_extent_tensors() {
  const Tensor rank_zero_result = Tensor::scalar(-2.5F) * -2.0F;

  expect(rank_zero_result.rank() == 0,
         "scalar multiplication changed rank-zero tensor rank");
  expect(rank_zero_result.numel() == 1,
         "scalar multiplication changed rank-zero tensor numel");
  expect(rank_zero_result.at() == 5.0F,
         "scalar multiplication computed an incorrect rank-zero value");
  ...
  expect_state(zero_extent_result, expected_shape, expected_strides, {},
               "zero-extent scalar multiplication changed shape",
               "zero-extent scalar multiplication changed strides",
               "zero-extent scalar multiplication created elements");
}

void test_scalar_multiply_reuses_rvalue_storage_in_both_orders() {
  ...
  expect(tensor_first.elements().data() == tensor_first_storage,
         "tensor-first scalar multiplication did not reuse rvalue storage");
  expect(std::ranges::equal(tensor_first.elements(),
                            Tensor::storage_type{4.0F, 8.0F, 12.0F}),
         "tensor-first rvalue scalar multiplication computed incorrect elements");
  ...
  expect(value_first.elements().data() == value_first_storage,
         "value-first scalar multiplication did not reuse rvalue storage");
  expect(std::ranges::equal(value_first.elements(),
                            Tensor::storage_type{-3.0F, 0.0F, 6.0F}),
         "value-first rvalue scalar multiplication computed incorrect elements");
  expect_empty_sentinel(value_first_source);
}

void test_scalar_multiply_rejects_sentinel_in_both_orders() {
  ...
  expect_throws<std::invalid_argument>(
      ...
      "tensor-first scalar multiplication accepted an empty sentinel",
      "tensor-first sentinel multiplication produced the wrong exception type");
  expect_empty_sentinel(tensor_first_source);
  ...
  expect_throws<std::invalid_argument>(
      ...
      "value-first scalar multiplication accepted an empty sentinel",
      "value-first sentinel multiplication produced the wrong exception type");
  expect_empty_sentinel(value_first_source);
}
...
int main() {
  try {
    test_scalar_multiply_supports_both_orders_without_changing_lvalue();
    test_scalar_multiply_accepts_convertible_numbers_in_both_orders();
    test_scalar_multiply_supports_rank_zero_and_zero_extent_tensors();
    test_scalar_multiply_reuses_rvalue_storage_in_both_orders();
    test_scalar_multiply_rejects_sentinel_in_both_orders();
  } catch (const std::exception& exception) {
    std::cerr << "FAILED: " << exception.what() << '\n';
    return EXIT_FAILURE;
  }

  std::cout << "All tensor scalar multiplication tests passed\n";
  return EXIT_SUCCESS;
}
